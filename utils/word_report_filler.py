...
class WordReportFiller:
    ...
    @staticmethod
    def insert_images_after_para(doc, image_paths: List[str], placeholder_para,
                                        max_width: float = 6.0):
    ...
    @staticmethod
    def fill_case_screenshots(config_manager, case_result: Dict[str, Any]) -> bool:
        """将测试结果填充到Word文档的对应表格中"""
        word_output_file = config_manager.get_screenshots_output_file()
        if os.path.exists(word_output_file):
            word_file = word_output_file
        else:
            word_file = config_manager.get_screenshots_word_file()
        ...
        try:
            for block in iter_block_items(doc):
                ...
                if isinstance(block, Paragraph):
                    header, test_case_id = get_id(block.text)
                    #print(test_case_id)
                    if test_case_id == case_result["case_id"]:
                        img_list = []
                        for item in case_result["execution_steps"]:
                            img_list = img_list + item["screenshot_path"]

                        WordReportFiller.insert_images_after_para(doc, img_list, block)

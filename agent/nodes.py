...
def run_test_step(state: TestState) -> Dict:
    ...
    try:
        ...
        '''
        # 启动子进程执行测试步骤的shell指令
        success, process_pid, errmsg, returncode = state.proc_manager.start_subprocess(
        ...
        log_file = state.proc_manager._get_subprocess_log_file(process_pid)
...
def run_fill_result(state: TestState) -> Dict:
    ...
    try:
        ...
        if step_idx < len(steps):
            state.add_log("所有测试步骤未全部执行完成")
            return {"current_step": -1}

        for step_idx, step in reversed(list(enumerate(steps, start=0))):  # start=1 表示索引从 0 开始（默认 0） enumerate(steps, start=0)
            #print(f"回填第 {step_idx + 1} 个步骤的结果：{step}") 
        ...
        if not state.errors:
            all_passed = all(
                #step["returncode"] 是process.returncode，而阻塞式进程返回的是None，没有参考意义
